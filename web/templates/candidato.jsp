<div class="container">
    ...
    <div>
        <table id="dt_candidato" class="mdl-data-table" cellspacing="0" width="100%">
            <thead>
                <tr>
                    <th></th>
                    <th>Nombres</th>
    ...
    <div>
        <form id="frmEliminarUsuario" action="" method="POST">
            <input type="hidden" id="cod_candidato" name="cod_candidato" value="">
...
<script>
    ...
    function eliminar() {
        $("#eliminar-usuario").on("click", function () {
            var cod_candidato = $("#frmEliminarUsuario #cod_candidato").val(),
            ...
            $.ajax({
                method: "POST",
                url: "<%= request.getContextPath()%>/ControlServlet?parAccion=guardarCandidato",
                data: {"cod_candidato": cod_candidato, "opcion": opcion}
            }).done(function (info) {
                console.log(info);
                var json_info = JSON.parse(info);
                mostrar_mensaje(json_info);
    ...
    function listar() {
        ...
        $("#cuadro1").slideDown("slow");
         ...
         var table = $("#dt_candidato").DataTable({   
            columnDefs: [ {
                orderable: false,
            ...
            "ajax": {
                "method": "POST",               
                "url": "<%= request.getContextPath()%>/ControlServlet?parAccion=listaCandidatos"
            },
            "columns": [
                {"defaultContent":""},
                {"data": "nombres"},
    ...
    function obtener_data_editar(table) {
        $("#btnEditar").on("click",function () {
            ...
            console.log(data);
            if(data!=null){
            $("#cod_candidato").val(data.cod_candidato);
    ...
    function obtener_id_eliminar(table) {
        $("#btnBorrar").on("click",function () {
            ...
            console.log(data);
            $("#frmEliminarUsuario #cod_candidato").val(data.cod_candidato);
